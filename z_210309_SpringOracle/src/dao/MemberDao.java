...
public class MemberDao {
	...
	//orcl DB연결을 위한 단계, DAO(Data Access Object)는 DB를 사용해 데이터를 조회하거나 조작하는 기능을 전담하도록 만든 오브젝트
	JdbcTemplate jdbcTemplate;	//Spring JDBC 접근방법 중 하나
	
	//DataSource 인터페이스활용, DB연결을 위한 단계
	...
	//DB 데이터 입력 시작
		public void insert(Member member) {
			...
			jdbcTemplate.update(new PreparedStatementCreator() {
				...
				@Override
				//jsp에서 많이 사용했었던 방식 (connection, preparedstatement, resultset 사용)
				public PreparedStatement createPreparedStatement(Connection con) throws SQLException {
					PreparedStatement stmt = con.prepareStatement(
							//자동생성하기로 한 id는 테이블 생성할 때 지정한 시퀀스 값을 넣어준다
			...
			Number keyValue = keyHolder.getKey();	//넘겨받은 keyholer의 값을 확인하기 위한 절차
			member.setId(keyValue.longValue());	//넘겨받은 keyholer의 값을 확인하기 위한 절차
